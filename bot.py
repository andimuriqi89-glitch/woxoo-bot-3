...
class CompteModal(discord.ui.Modal, title="🧾 Nouvelle fiche compte"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title=self.champ_titre.value, description=self.champ_description.value.strip() or None, color=color)
...
class BoostModal(discord.ui.Modal, title="⚡ Nouvelle fiche boost"):
    champ_titre = discord.ui.TextInput(label="Titre", placeholder="Ex: Boost x2 — 30 jours", required=True, max_length=256)
...
class AnnonceModal(discord.ui.Modal, title="📢 Nouvelle annonce"):
    champ_titre = discord.ui.TextInput(label="Titre", placeholder="Ex: 🔥 Nouvelle mise à jour !", required=True, max_length=256)
    champ_contenu = discord.ui.TextInput(label="Contenu", placeholder="Écris ton annonce ici...", style=discord.TextStyle.paragraph, required=True, max_length=3000)
    champ_image = discord.ui.TextInput(label="URL image (optionnel)", placeholder="https://...", required=False)
